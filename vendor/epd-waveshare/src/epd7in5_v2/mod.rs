    ...
    DELAY: DelayNs,
{
    fn init(&mut self, spi: &mut SPI, delay: &mut DELAY) -> Result<(), SPI::Error> {
        ...
        // LOCAL FORK: re-apply the explicit PowerSetting from upstream PR #228
        // ("Fix Epd7in5_V2 content fading out during DisplayFrame"). PR #249
        // dropped this line and relied on the panel's OTP default, which
        // reintroduces the slow fade / white-streak artifacts on our 7.5" V2
        // panel. The 2nd byte must be 0x07 (the buggy 0.6.0 release shipped
        // 0x17). Keep this whenever re-syncing with upstream.
    ...
    DELAY: DelayNs,
{
    ...
    fn update_partial_frame(
        ...
        if x % 8 != 0 || width % 8 != 0 {
            panic!("x and width should be byte-aligned (multiples of 8).")
        }
        ...
        let hrst_upper = (x >> 8) as u8;
        let hrst_lower = x as u8;
        let hred_upper = (x_end >> 8) as u8;
        let hred_lower = x_end as u8;
    ...
    DELAY: DelayNs,
{
    ...
    /// LOCAL FORK: enable the DC/DC converter without a full re-init, keeping
    /// the panel's RAM (OLD/NEW buffers), register settings and selected LUT
    /// intact. Pair with [`Epd7in5::power_off`].
    ///
    /// This is the lightweight alternative to [`WaveshareDisplay::wake_up`]
    /// (which re-runs `init` + a hardware reset, wiping the OLD buffer and the
    /// `set_lut` selection) and [`WaveshareDisplay::sleep`] (which additionally
    /// issues Deep Sleep, clearing RAM). Correct partial refresh needs the
    /// previous image to survive in the panel's OLD buffer between updates, so
    /// the controller powers the converter off/on around each refresh instead
    /// of deep-sleeping.
    ...
    /// LOCAL FORK: turn off the DC/DC converter to drop the high-voltage bias
    /// (which must not be held for long) while preserving RAM, settings and the
    /// selected LUT. The shown image persists because e-paper is bistable. Pair
    /// with [`Epd7in5::power_on`].
